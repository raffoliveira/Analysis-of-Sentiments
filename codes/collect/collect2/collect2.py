...
def getPublicCommentsFromID(facebook_id, exception_log):
    ...
    while (d_end <= 32):
        ...
        if d_begin == 31:
            d_end = 1
            begin = "2019-01-%d" % d_begin
            end = "2019-02-%d" % d_end
        else:
            begin = "2019-01-%d" % d_begin
            end = "2019-01-%d" % d_end

        outFile_comments_posts = gzip.open('posts_comments_haddad_' + str(begin) + '_' + str(end) + '.json.gz', 'wt')

        with gzip.GzipFile('posts_all_haddad_' + str(begin) + '_' + str(end) + '.json.gz', 'r') as file:
...
def getPublicPostFromID(facebook_id, log_file, begin, end):
    ...
    outFile_all_posts = gzip.open('posts_all_haddad_' + str(begin) + '_' + str(end) + '.json.gz', 'wt')
    outFile_ids_posts = gzip.open('posts_ids_haddad_' + str(begin) + '_' + str(end) + '.json.gz', 'wt')
...
def main():
    ...
    facebook_ids = ['211857482296579', '904277726319518']

    # 0 -> bolsonaro
    # 1 -> fernando haddad

    face_id = facebook_ids[1]
