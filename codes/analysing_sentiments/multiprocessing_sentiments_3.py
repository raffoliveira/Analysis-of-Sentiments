...
def analysing_Sentiments(id_month, profile_id):
...
def dividing_Into_Processes(profile_id):
	...
	for id_months in months:
		proc = mp.Process(target=analysing_Sentiments, args=(id_months, profile_id))
...
def main():
	...
	starttime = time.time()		
	dividing_Into_Processes(profiles[1])
	log_file.write('\nThe processing of {} took {} seconds.'.format(profiles[1], time.time()-starttime))
